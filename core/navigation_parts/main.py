...
def sidebar_nav():
    ...
    """
    _init_sidebar_state()
    if st.session_state.get("tab_choice") == "Research":
        st.session_state["tab_choice"] = "Lunch"
        st.session_state["home_inner_tab"] = "Research"

    with st.sidebar:
        st.markdown("### ⚡ Quant Control")
        st.caption("Simple sidebar: Lunch/Home, Other, Connector, Timer. Research is now inside Lunch/Home.")

        # 1) Lunch button
        home_active = st.session_state.get("tab_choice", "Lunch") == "Lunch"
        if st.button(("✅ 🍱 Lunch" if home_active else "🍱 Lunch"), use_container_width=True, key="nav_lunch_v24"):
            _open_tab("Lunch")

        # 2) Research moved inside Lunch/Home as an inner tab.
        try:
        ...
        # 6) UI/UX mode buttons, aligned like tab-choice buttons
    ...
    return st.session_state.get("tab_choice", DEFAULT_TABS[0] if DEFAULT_TABS else "Lunch")
