...
def install(ns: dict) -> None:
    ...
    def _impact_article(article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        eur_pos = ["ecb hawkish", "ecb hike", "euro rises", "euro stronger", "eurozone pmi beat", "german pmi beat", "eurozone inflation hot", "euro area growth"]
        eur_neg = ["ecb cut", "ecb dovish", "euro falls", "euro weaker", "eurozone pmi miss", "german pmi miss", "eurozone recession", "french risk", "italy risk"]
        usd_pos = ["fed hawkish", "fed hike", "dollar rises", "dollar stronger", "nfp beat", "jobs beat", "cpi hot", "treasury yields rise", "retail sales beat"]
        usd_neg = ["fed cut", "fed dovish", "dollar falls", "dollar weaker", "nfp miss", "jobs miss", "cpi cool", "treasury yields fall", "retail sales miss"]
        ...
        if " euro " in text_clean or " eur " in text_clean or " eurozone " in text_clean:
            eur_score += _sentiment_score(text_clean) * 0.55
        if " dollar " in text_clean or " usd " in text_clean or " fed " in text_clean or " treasury " in text_clean:
            usd_score += _sentiment_score(text_clean) * 0.55
