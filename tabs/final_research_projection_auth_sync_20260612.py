...
def install(ns: dict) -> None:
    ...
    def _mobile_css() -> None:
        big = bool(st.session_state.get("mobile_app_big_mode_20260612", False) or st.session_state.get("phone_mode", False))
        mult = 1.28 if big else 1.0
        st.markdown(f"""
        <style>
    ...
    def _chart_powerbi_synced(d: pd.DataFrame, base_pred: pd.DataFrame, synced_proj: pd.DataFrame, title: str) -> None:
        ...
        if isinstance(base_pred, pd.DataFrame) and not base_pred.empty:
            ...
            if tcol and ccol:
                ...
                fig.add_trace(go.Scatter(x=pp[tcol], y=pd.to_numeric(pp[ccol], errors="coerce"), mode="lines", name="Original PowerBI path", opacity=.45))
        if isinstance(synced_proj, pd.DataFrame) and not synced_proj.empty:
            fig.add_trace(go.Scatter(x=synced_proj["time"], y=synced_proj["Accuracy Adjusted Price"], mode="lines+markers", name="SYNCED accuracy-adjusted path", line={"width": 4}))
        ...
        fig.update_layout(height=690, margin=dict(l=6, r=6, t=54, b=6), title=title, xaxis_rangeslider_visible=False, legend=dict(orientation="h"))
    ...
    def _combined_merged_section() -> None:
        with st.expander("🧠 Open / Close — Final Synced Intelligence: ML Tables + KNN/Greedy + News/NLP + Quant Structure + Research", expanded=False):
            ...
            if run_cols[2].button("📱 Toggle Real Mobile App UI", use_container_width=True, key=f"toggle_mobile_{UNIQUE}"):
                st.session_state["mobile_app_big_mode_20260612"] = not bool(st.session_state.get("mobile_app_big_mode_20260612", False))
                st.session_state["phone_mode"] = bool(st.session_state.get("mobile_app_big_mode_20260612"))
                st.rerun()
    ...
    def _render_data_visualization_final() -> None:
        ...
        c = st.columns([1.05, .72, .72, .72, .72, .72])
        run = c[0].button("▶ Run PowerBI + Synced Projection", use_container_width=True, key=f"dv_run_{UNIQUE}")
        rows = c[1].slider("Rows used", 800, 12000, int(st.session_state.get("dv_pp_rows_v6", 6000)), 400, key=f"rows_{UNIQUE}")
        horizon = c[2].slider("Future candles", 6, 60, int(st.session_state.get("dv_pp_horizon_v6", 24)), 6, key=f"horizon_{UNIQUE}")
        yellow = c[3].slider("Yellow prev hours", 1, 12, int(st.session_state.get("yellow_horizon_v6", 6)), 1, key=f"yellow_{UNIQUE}")
        mode = c[4].selectbox("Projection choice", ["Balanced", "Trend follow", "Pullback safer"], key=f"mode_{UNIQUE}")
        risk = c[5].selectbox("Risk filter", ["Low", "Medium", "High"], index=1, key=f"risk_{UNIQUE}")
        m = st.columns(3)
        min_days = m[0].slider("Regime min days", 3, 21, int(st.session_state.get("dv_pp_min_days_v6", 5)), 1, key=f"min_days_{UNIQUE}")
        bt = m[1].slider("Backtest bars", 60, 360, int(st.session_state.get("dv_pp_bt_v6", 180)), 20, key=f"bt_{UNIQUE}")
        if m[2].button("📱 Real Mobile Big UI", use_container_width=True, key=f"mobile_big_{UNIQUE}"):
            st.session_state["mobile_app_big_mode_20260612"] = not bool(st.session_state.get("mobile_app_big_mode_20260612", False))
            st.session_state["phone_mode"] = bool(st.session_state.get("mobile_app_big_mode_20260612"))
            st.rerun()
        if run:
    ...
    def _render_research_inner_home() -> None:
        ...
        finnhub = c[2].text_input("Finnhub API key", value=st.session_state.get("research_finnhub_key", ""), type="password", key=f"research_finnhub_home_{UNIQUE}")
        ...
        selected = st.radio("Research inner tab", ["Data Analysis", "Data Mining", "NLP", "Library Status", "Copy"], horizontal=True, key=f"research_inner_home_{UNIQUE}")
        ...
        if selected == "Data Analysis":
            ...
            st.info("NLP direction = EUR impact - USD impact. It only confirms/conflicts with PowerBI, never overrides it.")
    ...
    def _top_home_copy() -> None:
        ...
        c = st.columns(2)
        with c[0]:
            _copy_button("📋 Copy Necessary 100 Lines — Current H1", compact, f"home_top_copy_100_{UNIQUE}")
        with c[1]:
            _copy_button("📋 Copy Full Home H1", full, f"home_top_copy_full_{UNIQUE}")
