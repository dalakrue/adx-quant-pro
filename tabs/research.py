...
def _score_news(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    eur_pos, eur_neg = ("euro rises", "ecb hawkish", "pmi beat"), ("euro falls", "ecb dovish", "pmi miss", "recession")
    usd_pos, usd_neg = ("dollar rises", "fed hawkish", "nfp beat", "cpi hot", "yields rise"), ("dollar falls", "fed cut", "fed dovish", "nfp miss", "cpi cool")
    out = []
    for r in rows:
        ...
        out.append({"Title": str(r.get("headline") or "")[:160], "Source": str(r.get("source") or "news"), "EUR Impact": round(eur, 2), "USD Impact": round(usd, 2), "VADER": round(sent, 3), "News Direction": nd, "Published": str(r.get("publishedDate") or "")[:35]})
